...
async def get_contacts_paginated(
    ...
    if contact_type == "influencer" and len(contacts) < page_size:
        ...
        # Note: The 'influencers' collection has been migrated to 'contacts' with contact_type='influencer'
        # No need to query the deprecated collection anymore
...
async def create_contact(data: ContactCreate, user: dict = Depends(get_marketing_auth())):
    ...
    contact_doc = {
        "id": contact_id,
        **data.model_dump(),
        "status": getattr(data, 'status', None) or "identified",
        "pipeline_stage": getattr(data, 'status', None) or "identified",  # Sync pipeline_stage with status
...
async def add_influencer_to_campaign(
    ...
    # Verify influencer exists in contacts collection (influencers collection has been migrated)
...
async def migrate_influencers_to_contacts(
    ...
    """
    Migration endpoint - DEPRECATED
    The 'influencers' collection has been fully migrated to 'contacts'.
    This endpoint is kept for backwards compatibility but no longer performs any migration.
    """
    return {
        "status": "completed",
        "message": "Migration already completed. All influencers are now stored in the 'contacts' collection with contact_type='influencer'.",
        "note": "The original 'influencers' collection has been backed up and dropped."
    }
