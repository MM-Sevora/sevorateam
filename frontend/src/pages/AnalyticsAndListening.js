  ...
  return (
    <div className="space-y-6 animate-fade-in" data-testid="analytics-page">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-heading font-bold text-white tracking-tight">Analytics & Listening</h1>
          <p className="text-zinc-400 mt-1">Measure performance, track sentiment, and monitor your brand</p>
        </div>
        {tab === 'analytics' && (
          <button onClick={handleExportCSV} className="bg-zinc-800 hover:bg-zinc-700 text-white border border-white/10 rounded-lg font-medium px-4 py-2 text-sm flex items-center gap-2 transition-all" data-testid="export-csv">
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-zinc-900/50 border border-white/5 rounded-xl p-5">
              <h3 className="text-sm font-heading font-semibold text-white mb-4">Posts by Platform</h3>
              ...
              ) : <p className="text-sm text-zinc-500 text-center py-8">No published posts to analyze</p>}
            </div>

            <div className="bg-zinc-900/50 border border-white/5 rounded-xl p-5">
              <h3 className="text-sm font-heading font-semibold text-white mb-4 flex items-center gap-2"><Zap className="w-4 h-4 text-amber-400" /> Industry Benchmarks</h3>
              <div className="space-y-3">
                {Object.entries(analytics?.benchmarks || {}).map(([platform, bench]) => {
                  ...
                  return (
                    <div key={platform} className="flex items-center gap-3 p-2.5 bg-zinc-800/60 rounded-lg">
                      {Icon && <Icon className="w-4 h-4" style={{ color: meta?.color }} />}
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-white capitalize">{platform}</p>
                        <p className="text-[10px] text-zinc-500">Benchmark: {bench.avg_engagement_rate}% eng, {bench.avg_likes_per_post} likes/post</p>
                      </div>
                      {myStats && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[{ title: 'Top Performing', posts: analytics?.best_posts, icon: TrendingUp, color: 'text-emerald-400' },
              ...
              <div key={section.title} className="bg-zinc-900/50 border border-white/5 rounded-xl p-5">
                ...
                {section.posts?.length > 0 ? section.posts.map((p, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-start gap-2 p-2 bg-zinc-800/60 rounded-lg mb-2">
                      {Icon && <Icon className="w-3.5 h-3.5 mt-0.5 flex-shrink-0" style={{ color: meta?.color }} />}
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-zinc-300 line-clamp-1">{p.content}</p>
        ...
        <div className="space-y-5">
          <div className="bg-zinc-900/50 border border-white/5 rounded-xl p-5">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleListen} disabled={listening || !listenQuery.trim()}
                className="bg-accent-violet hover:bg-accent-violet-hover text-white rounded-lg font-medium px-6 py-2.5 text-sm flex items-center gap-2 disabled:opacity-50" data-testid="listen-button"
          ...
          {listenResult && (
            <div className="space-y-4 animate-slide-up">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-zinc-900/50 border border-white/5 rounded-xl p-5 flex flex-col items-center">
                  <p className="text-[10px] text-zinc-500 uppercase tracking-wider mb-3">Overall Sentiment</p>
                  <div className="relative w-24 h-24 mb-3">
                    ...
                    <span className="absolute inset-0 flex items-center justify-center text-xl font-heading font-bold text-white">{listenResult.sentiment_score || 50}</span>
                  </div>
                ...
                <div className="bg-zinc-900/50 border border-white/5 rounded-xl p-5">
                  <p className="text-[10px] text-zinc-500 uppercase tracking-wider mb-3">Sentiment Breakdown</p>
                  {listenResult.sentiment_breakdown && (
                    <div className="space-y-3">
                      {Object.entries(listenResult.sentiment_breakdown).map(([key, val]) => (
                        <div key={key} className="flex items-center gap-3">
                          <span className="text-xs text-zinc-400 w-16 capitalize">{key}</span>
                          <div className="flex-1 h-2 rounded-full bg-zinc-800 overflow-hidden">
                            <div className="h-full rounded-full transition-all" style={{ width: `${val}%`, backgroundColor: SENTIMENT_COLORS[key] || '#666' }} />
                          </div>
                          <span className="text-xs text-zinc-400 w-10 text-right">{val}%</span>
                        </div>
                      ))}
                    </div>
                  )}
                  {listenResult.audience_mood && <p className="text-xs text-zinc-400 mt-3 bg-zinc-800/60 p-2 rounded-lg">{listenResult.audience_mood}</p>}
                </div>

                <div className="bg-zinc-900/50 border border-white/5 rounded-xl p-5">
                  <p className="text-[10px] text-zinc-500 uppercase tracking-wider mb-3">Brand Health</p>
                  <div className="text-center">
                    <p className="text-4xl font-heading font-bold text-white mb-1">{listenResult.brand_health_score || 0}</p>
                    <p className="text-xs text-zinc-500">/100</p>
                  </div>
                  {listenResult.hashtag_analysis?.length > 0 && (
                    <div className="mt-4">
                      <p className="text-[10px] text-zinc-500 mb-2">Related Hashtags</p>
                      <div className="flex flex-wrap gap-1">{listenResult.hashtag_analysis.slice(0, 5).map((h, i) => (
                        <span key={i} className="text-[10px] bg-accent-violet/10 text-accent-violet px-2 py-0.5 rounded">#{h.tag || h}</span>
                      ))}</div>
                    </div>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {listenResult.trending_topics?.length > 0 && (
                  <div className="bg-zinc-900/50 border border-white/5 rounded-xl p-5">
                    <h3 className="text-sm font-heading font-semibold text-white mb-3 flex items-center gap-2"><TrendingUp className="w-4 h-4 text-accent-cyan" /> Trending Topics</h3>
                    <div className="space-y-2">{listenResult.trending_topics.map((t, i) => (
                      <div key={i} className="p-2.5 bg-zinc-800/60 rounded-lg">
                ...
                {listenResult.key_mentions?.length > 0 && (
                  <div className="bg-zinc-900/50 border border-white/5 rounded-xl p-5">
                    <h3 className="text-sm font-heading font-semibold text-white mb-3 flex items-center gap-2"><MessageSquare className="w-4 h-4 text-pink-400" /> Key Mentions</h3>
                    <div className="space-y-2">{listenResult.key_mentions.map((m, i) => (
                      <div key={i} className="p-2.5 bg-zinc-800/60 rounded-lg">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-[10px] text-zinc-500 capitalize">{m.platform}</span>
              ...
              {listenResult.recommendations?.length > 0 && (
                <div className="bg-zinc-900/50 border border-white/5 rounded-xl p-5">
                  <h3 className="text-sm font-heading font-semibold text-white mb-3 flex items-center gap-2"><Sparkles className="w-4 h-4 text-amber-400" /> AI Recommendations</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {listenResult.recommendations.map((r, i) => (
                      <div key={i} className="flex items-start gap-2.5 p-3 bg-zinc-800/60 rounded-lg">
                        <CheckCircle className="w-4 h-4 text-accent-violet mt-0.5 flex-shrink-0" />
          ...
          {!listenResult && !listening && (
            <div className="text-center py-16">
              ...
              <h3 className="text-lg font-heading font-semibold text-zinc-400">Monitor Your Brand</h3>
              <p className="text-sm text-zinc-600 mt-1">Enter your brand name, product, or any topic to analyze sentiment and trending conversations</p>
            </div>
          )}
        </div>
      )}
