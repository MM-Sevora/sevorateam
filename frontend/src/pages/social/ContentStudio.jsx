...
function ScoreRing({ score, size = 80 }) {
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <span className="absolute inset-0 flex items-center justify-center text-lg font-heading font-bold text-gray-900">{score}</span>
    </div>
  );
}
    ...
    <div className="space-y-6 animate-fade-in p-8" data-testid="content-studio-page">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Ideate, create, refine, review, and queue content</p>
        </div>
      </div>
      ...
      <div className="flex gap-1 p-1 bg-white rounded-xl border border-gray-200">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${tab === t.key ? 'bg-violet-100 text-violet-700' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}
        ...
        <div className="flex gap-1.5">
          {platformOptions.map(p => (
            <button key={p.value} onClick={() => setPlatform(p.value)}
              className={`p-2.5 rounded-lg transition-all border ${platform === p.value ? 'bg-gray-100 border-gray-300' : 'border-transparent hover:bg-gray-50'}`}
              data-testid={`studio-platform-${p.value}`}
            ><p.icon className="w-4 h-4" style={{ color: platform === p.value ? p.color : '#71717a' }} /></button>
          ))}
        </div>
        <select value={tone} onChange={(e) => setTone(e.target.value)}
          className="bg-white border border-gray-300 rounded-lg py-2 px-3 text-xs text-gray-900"
      ...
      {tab === 'create' && (
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-5" style={{ minHeight: 'calc(100vh - 280px)' }}>
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-white backdrop-blur-md border border-gray-200 rounded-xl p-5">
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                {[
                  ...
                  <button key={f.key} onClick={() => { setContentFormat(f.key); setContentType(f.key === 'text' ? 'post' : f.key); }}
                    className={`px-2.5 py-1.5 rounded-lg text-[10px] font-medium border transition-all ${contentFormat === f.key ? 'bg-violet-600/10 border-violet-600/20 text-violet-600' : 'border-gray-200 text-gray-500 hover:text-white hover:bg-gray-50'}`}
                  >{f.icon} {f.label}</button>
                ))}
              </div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] text-gray-500 uppercase tracking-wider">Topic</span>
                <button onClick={() => { setShowIdeas(!showIdeas); if (!showIdeas && ideas.length === 0) generateIdeas(); }}
                  className={`text-[10px] px-2.5 py-1 rounded-lg flex items-center gap-1 transition-all ${showIdeas ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : 'text-gray-500 hover:text-white border border-gray-200 hover:bg-gray-50'}`}
                ><Lightbulb className="w-3 h-3" /> {showIdeas ? 'Hide Ideas' : 'Get Ideas'}</button>
              </div>
              <textarea value={topic} onChange={(e) => setTopic(e.target.value)}
                className="w-full bg-gray-50 border border-gray-300 focus:border-violet-600/50 focus:ring-2 focus:ring-violet-600/20 rounded-lg py-3 px-4 text-sm text-white placeholder-zinc-500 resize-none"
                ...
                <div className="mt-3 space-y-1.5 max-h-40 overflow-y-auto">
                  ...
                  ideas.length > 0 ? ideas.map((idea, i) => (
                    <button key={i} onClick={() => { setTopic(idea.title + ' - ' + idea.description); setShowIdeas(false); }}
                      className="w-full text-left p-2.5 bg-gray-50 hover:bg-gray-200/60 rounded-lg border border-gray-200 hover:border-amber-500/20 transition-all"
                    >
                  ...
                  )) : <p className="text-[10px] text-gray-400 text-center py-2">Click to generate AI ideas for your platform</p>}
                </div>
              )}
              <div className="flex items-center gap-3 mt-3">
                <select value={contentType} onChange={(e) => setContentType(e.target.value)}
                  className="bg-gray-50 border border-gray-300 rounded-lg py-2 px-3 text-xs text-white"
                ...
                <button onClick={generateContent} disabled={loadingText || !topic.trim()}
                  className="bg-violet-600 hover:bg-violet-600-hover text-white shadow-[0_0_15px_rgba(124,58,237,0.3)] rounded-lg font-medium px-5 py-2 text-sm flex items-center gap-2 disabled:opacity-50 transition-all"
                ...
                <button onClick={generateImage} disabled={loadingImage || !topic.trim()}
                  className="bg-gray-100 hover:bg-gray-200 text-white border border-gray-300 rounded-lg font-medium px-5 py-2 text-sm flex items-center gap-2 disabled:opacity-50 transition-all"
            ...
            {generatedContent && (
              <>
                ...
                <div className="bg-white backdrop-blur-md border border-amber-500/10 rounded-xl p-4">
                  <h4 className="text-xs font-heading font-semibold text-gray-900 mb-3 flex items-center gap-2"><Sparkles className="w-3.5 h-3.5 text-amber-400" /> AI Refine</h4>
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {[
                      ...
                      <button key={tool.action} onClick={() => refineContent(tool.action)} disabled={!!refining}
                        className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[10px] font-medium border transition-all ${refining === tool.action ? 'bg-amber-500/10 border-amber-500/20 text-amber-400' : 'border-gray-300 text-gray-500 hover:text-white hover:bg-gray-50'}`}
                      >{refining === tool.action ? <Loader2 className="w-3 h-3 animate-spin" /> : <tool.icon className="w-3 h-3" />} {tool.label}</button>
                    ))}
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    <span className="text-[9px] text-gray-400 mr-1 self-center">Tone:</span>
                    {['casual', 'professional', 'humorous', 'inspirational'].map(t => (
                      <button key={t} onClick={() => refineContent('change_tone', { tone: t })} disabled={!!refining} className="text-[10px] px-2 py-1 rounded border border-gray-200 text-gray-500 hover:text-white hover:bg-gray-50 capitalize">{t}</button>
                    ))}
                    <span className="text-[9px] text-gray-400 ml-2 mr-1 self-center">Lang:</span>
                    {['Spanish', 'Hindi', 'French'].map(l => (
                      <button key={l} onClick={() => refineContent('translate', { language: l })} disabled={!!refining} className="text-[10px] px-2 py-1 rounded border border-gray-200 text-gray-500 hover:text-white hover:bg-gray-50">{l}</button>
                    ))}
                  </div>
                </div>

                {/* Quality + Team Review + Send to Queue */}
                <div className="bg-white backdrop-blur-md border border-violet-600/10 rounded-xl p-4 space-y-3">
                  {/* Step 1: Quality Check */}
                  <div className="flex items-center gap-2">
                    <span className="w-5 h-5 rounded-full bg-violet-600/15 text-violet-600 text-[10px] font-bold flex items-center justify-center">1</span>
                    <button onClick={checkQuality} disabled={checkingQuality} className="text-[10px] bg-gray-100 hover:bg-gray-200 text-white border border-gray-300 rounded-lg px-3 py-1.5 flex items-center gap-1 disabled:opacity-50">
                  ...
                  {qualityScore?.suggestions && (
                    <div className="space-y-1 pl-7">{qualityScore.suggestions.slice(0, 2).map((s, i) => (
                      <p key={i} className="text-[10px] text-gray-500 flex items-start gap-1.5"><AlertTriangle className="w-3 h-3 text-amber-400 mt-0.5 flex-shrink-0" />{s}</p>
                    ))}</div>
                  )}
                  ...
                  <div className="border-t border-gray-200 pt-3">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-[10px] text-gray-500">Team Review (optional)</span>
                    </div>
                    <div className="pl-7 flex gap-2">
                      <input type="text" value={reviewNote} onChange={(e) => setReviewNote(e.target.value)}
                        className="flex-1 bg-gray-50 border border-gray-300 rounded-lg py-1.5 px-3 text-[10px] text-white placeholder-zinc-600"
                  ...
                  <div className="border-t border-gray-200 pt-3">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-[10px] text-gray-500">Send to Queue</span>
                    </div>
                    <div className="pl-7 flex gap-2">
                      <button onClick={() => saveAsPost('draft')} disabled={savingPost} className="flex-1 bg-gray-100 hover:bg-gray-200 text-white border border-gray-300 rounded-lg px-3 py-2 text-[10px] flex items-center justify-center gap-1.5 disabled:opacity-50 transition-all">
                        <Download className="w-3 h-3" /> Save Draft
                      </button>
                      <button onClick={() => saveAsPost('scheduled')} disabled={savingPost} className="flex-1 bg-violet-600 hover:bg-violet-600-hover text-white rounded-lg px-3 py-2 text-[10px] flex items-center justify-center gap-1.5 disabled:opacity-50 shadow-[0_0_10px_rgba(124,58,237,0.2)] transition-all">
                        <Send className="w-3 h-3" /> Add to Queue
                      </button>
                    </div>
                    <p className="text-[9px] text-gray-400 mt-1.5 pl-7">Then go to Posts & Schedule to set timing and publish</p>
                  </div>
                </div>
              </>
            )}
          ...
          <div className="lg:col-span-3">
            {generatedContent ? (
              <div className="bg-white backdrop-blur-md border border-gray-200 rounded-xl overflow-hidden h-full flex flex-col">
                <div className="flex items-center justify-between p-4 border-b border-gray-200">
                  <h3 className="text-sm font-heading font-semibold text-gray-900">Generated Content</h3>
                  <button onClick={copyContent} className="text-gray-500 hover:text-white p-1.5 rounded-lg hover:bg-gray-50" data-testid="studio-copy">
                    {copied ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />}
                  </button>
                </div>
                <div className="flex-1 overflow-y-auto p-4">
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200 mb-3">
                    <p className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed" data-testid="studio-generated-text">{generatedContent.content}</p>
                  </div>
                  {generatedContent.hashtags?.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {generatedContent.hashtags.map((tag, i) => <span key={i} className="text-xs bg-violet-600/10 text-violet-600 px-2 py-1 rounded-md">#{tag.replace('#', '')}</span>)}
                    </div>
                  )}
                  {generatedContent.call_to_action && (
                    <p className="text-xs text-gray-500 mb-4"><span className="text-gray-500 font-medium">CTA:</span> {generatedContent.call_to_action}</p>
                  )}
                  {generatedImage && (
                    <div className="mb-4">
                      <img src={generatedImage} alt="Generated" className="w-full rounded-lg border border-gray-200" data-testid="studio-image" />
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-16 text-center h-full flex flex-col items-center justify-center">
                ...
                <h3 className="text-lg font-heading font-semibold text-gray-500">Create Your Content</h3>
                <p className="text-sm text-gray-400 mt-1">Enter a topic on the left and click Generate</p>
                <p className="text-xs text-gray-300 mt-3">Then use AI Refine tools to perfect it before adding to queue</p>
              </div>
            ) : (
              <div className="bg-white border border-gray-200 rounded-xl p-16 text-center h-full flex flex-col items-center justify-center">
...
function ReviewTab() {
  ...
  if (loading) return <div className="flex justify-center py-12"><Loader2 className="w-6 h-6 text-violet-600 animate-spin" /></div>;
  ...
  return (
    <div className="space-y-4">
      ...
      {sections.map(section => section.items.length > 0 && (
        <div key={section.key} className="bg-white border border-gray-200 rounded-xl p-5">
          <h3 className="text-sm font-heading font-semibold text-gray-900 mb-3 flex items-center gap-2">
            ...
            <span className="text-[10px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">{section.items.length}</span>
          </h3>
          <div className="space-y-2">
            {section.items.slice(0, 10).map(a => (
              <div key={a.approval_id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className="text-xs font-medium text-white capitalize">{a.post_platform}</span>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full ${a.status === 'approved' ? 'bg-emerald-500/10 text-emerald-400' : a.status === 'rejected' ? 'bg-red-500/10 text-red-400' : a.status === 'changes_requested' ? 'bg-violet-600/10 text-violet-600' : 'bg-amber-500/10 text-amber-400'}`}>{a.status.replace('_', ' ')}</span>
                    </div>
                    <p className="text-sm text-gray-700 line-clamp-2">{a.post_content || a.content_preview}</p>
                    {a.reviews?.length > 0 && (
                  ...
                  <div className="flex flex-col gap-2 flex-shrink-0">
                    ...
                    {section.key === 'needs_review' && (
                      <>
                        <input type="text" value={reviewFeedback[a.approval_id] || ''} onChange={(e) => setReviewFeedback(prev => ({ ...prev, [a.approval_id]: e.target.value }))}
                          className="bg-gray-50 border border-gray-300 rounded-lg py-1.5 px-3 text-xs text-white placeholder-zinc-600 w-44" placeholder="Feedback..." />
                    ...
                    {section.key === 'needs_edit' && (
                      <button onClick={() => handleResubmit(a.approval_id)} className="text-[10px] bg-violet-600 hover:bg-violet-600-hover text-white rounded-lg px-3 py-2 flex items-center gap-1"><RefreshCw className="w-3 h-3" /> Resubmit</button>
                    )}
                  </div>
                </div>
              </div>
      ...
      {!actionable || Object.values(actionable).every(v => v.length === 0) && (
        <div className="text-center py-16"><CheckCircle className="w-12 h-12 text-gray-300 mx-auto mb-4" /><h3 className="text-lg font-heading font-semibold text-gray-500">No Reviews</h3><p className="text-sm text-gray-400 mt-1">Submit content for review from Create & Refine tab</p></div>
      )}
    </div>
  );
}
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-heading font-semibold text-gray-900 flex items-center gap-2"><Layers className="w-4 h-4 text-violet-600" /> Content Pillars</h3>
          <button onClick={() => setShowAddPillar(!showAddPillar)} className="text-[10px] text-violet-600 hover:text-violet-600-hover flex items-center gap-1"><Plus className="w-3 h-3" /> Add Pillar</button>
        </div>
        {showAddPillar && (
          <div className="flex gap-2 mb-3 flex-wrap p-3 bg-gray-50 rounded-lg">
            <input type="text" value={newPillar.name} onChange={(e) => setNewPillar(prev => ({ ...prev, name: e.target.value }))} className="bg-gray-50 border border-gray-300 rounded-lg py-2 px-3 text-xs text-white placeholder-zinc-500 w-32" placeholder="Pillar name" />
            <input type="text" value={newPillar.description} onChange={(e) => setNewPillar(prev => ({ ...prev, description: e.target.value }))} className="flex-1 bg-gray-50 border border-gray-300 rounded-lg py-2 px-3 text-xs text-white placeholder-zinc-500 min-w-[150px]" placeholder="Description..." />
            <input type="number" value={newPillar.target_percentage} onChange={(e) => setNewPillar(prev => ({ ...prev, target_percentage: parseInt(e.target.value) || 0 }))} className="bg-gray-50 border border-gray-300 rounded-lg py-2 px-3 text-xs text-white w-16" />
            ...
            <button onClick={handleAddPillar} disabled={savingPillar} className="text-[10px] bg-violet-600 text-white rounded-lg px-3 py-2 disabled:opacity-50">{savingPillar ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />}</button>
          </div>
        )}
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setSelectedPillar('')} className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${!selectedPillar ? 'bg-gray-100 border-gray-300 text-white' : 'border-gray-200 text-gray-500 hover:text-white'}`}>All</button>
          {pillars.map(p => (
            <div key={p.pillar_id} className="flex items-center gap-1">
              <button onClick={() => setSelectedPillar(p.name)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all flex items-center gap-1.5 ${selectedPillar === p.name ? 'bg-gray-100 border-gray-300 text-white' : 'border-gray-200 text-gray-500 hover:text-white'}`}
              ><div className="w-2 h-2 rounded-full" style={{ backgroundColor: p.color }} /> {p.name} <span className="text-[9px] text-gray-400">{p.target_percentage}%</span></button>
              <button onClick={() => handleDeletePillar(p.pillar_id)} className="p-1 text-gray-300 hover:text-red-400"><X className="w-3 h-3" /></button>
            </div>
          ))}
        </div>
        {pillars.length > 0 && (
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 flex gap-3">
        ...
        <button onClick={selectedPillar ? generateByPillar : generateIdeas} disabled={loadingIdeas}
          className="bg-violet-600 hover:bg-violet-600-hover text-white rounded-lg font-medium px-5 py-2.5 text-sm flex items-center gap-2 disabled:opacity-50"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {ideas.map((idea, i) => (
            <div key={i} onClick={() => { setTopic(idea.title + ' - ' + idea.description); setTab('create'); }}
              className="bg-white border border-gray-200 rounded-xl p-5 hover:border-violet-600/20 transition-all hover:-translate-y-0.5 cursor-pointer group"
            >
              {idea.pillar && <span className="text-[9px] bg-violet-600/10 text-violet-600 px-2 py-0.5 rounded-full mb-2 inline-block">{idea.pillar}</span>}
              <h3 className="text-sm font-heading font-semibold text-gray-900 mb-2">{idea.title}</h3>
              <p className="text-xs text-gray-500 mb-3 leading-relaxed">{idea.description}</p>
              {idea.hashtags?.length > 0 && <div className="flex flex-wrap gap-1 mb-2">{idea.hashtags.slice(0, 4).map((t, j) => <span key={j} className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded">#{t.replace('#','')}</span>)}</div>}
              ...
              <p className="text-[9px] text-violet-600 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">Click to use in Create &rarr;</p>
            </div>
          ))}
        </div>
      ) : !loadingIdeas && (
        <div className="text-center py-16"><Lightbulb className="w-12 h-12 text-gray-300 mx-auto mb-4" /><h3 className="text-lg font-heading font-semibold text-gray-500">Get AI Content Ideas</h3><p className="text-sm text-gray-400 mt-1">{selectedPillar ? `Select "${selectedPillar}" pillar and generate ideas` : 'Select a platform, set up pillars, and generate ideas'}</p></div>
      )}
    </div>
  );
}
