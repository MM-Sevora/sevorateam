  ...
  return (
    <div className="space-y-6 animate-fade-in" data-testid="autopilot-page">
      <div>
        ...
        <p className="text-[#5D4A3A] mt-1">AI generates and schedules a full week of content across your platforms</p>
      </div>

      <div className="bg-white backdrop-blur-md border border-[#E8D5C4] rounded-xl p-6" data-testid="autopilot-config">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            ...
            <select value={tone} onChange={(e) => setTone(e.target.value)}
              className="w-full bg-[#F5EDE5] border border-[#D4BBA6] rounded-lg py-2.5 px-4 text-sm text-white transition-all" data-testid="autopilot-tone"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[#4A3728] mb-2">Platforms</label>
              ...
              {Object.entries(platformMeta).map(([key, meta]) => {
                ...
                return (
                  <button key={key} onClick={() => togglePlatform(key)}
                    className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium border transition-all ${active ? 'bg-[#E8D5C4] border-[#D4BBA6] text-white' : 'border-[#E8D5C4] text-[#5D4A3A]'}`}
      ...
      {Object.keys(groupedByDay).length > 0 && (
        <div className="space-y-4" data-testid="autopilot-calendar">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-heading font-semibold text-[#4A3728] flex items-center gap-2">
              <Calendar className="w-5 h-5 text-amber-600" /> Content Calendar ({scheduledPosts.length} posts)
            </h2>
          </div>
          {Object.entries(groupedByDay).sort().map(([day, posts]) => (
            <div key={day} className="space-y-2">
              <h3 className="text-sm font-heading font-semibold text-[#4A3728] flex items-center gap-2">
                <Clock className="w-4 h-4 text-[#5D4A3A]" />
                {new Date(day + 'T00:00:00').toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}
                <span className="text-xs text-[#5D4A3A] font-normal">({posts.length} posts)</span>
              </h3>
              {posts.map(post => {
                ...
                return (
                  <div key={post.post_id} className="bg-white backdrop-blur-md border border-[#E8D5C4] rounded-xl p-4 hover:border-[#D4BBA6] transition-colors" data-testid={`autopilot-post-${post.post_id}`}>
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`text-[10px] px-2 py-0.5 rounded-full ${post.status === 'published' ? 'bg-stone-600/10 text-stone-400' : 'bg-amber-800/10 text-amber-600'}`}>
                            {post.status}
                          </span>
                          <span className="text-[10px] text-[#5D4A3A]">{post.scheduled_at}</span>
                        </div>
                        <p className="text-sm text-[#4A3728] leading-relaxed line-clamp-3">{post.content}</p>
                        {post.hashtags?.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-2">
                            {post.hashtags.slice(0, 5).map((tag, i) => (
                              <span key={i} className="text-[10px] bg-[#E8D5C4] text-[#5D4A3A] px-1.5 py-0.5 rounded">#{tag.replace('#','')}</span>
                            ))}
                          </div>
                        )}
                        {result && (
                          <div className={`mt-2 text-xs p-2 rounded-lg ${result.success ? 'bg-stone-600/10 text-stone-400' : 'bg-red-500/10 text-red-400'}`}>
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        {post.status !== 'published' && (
                          <button onClick={() => handlePublishPost(post)} disabled={!!publishing}
                            className="p-2 rounded-lg hover:bg-stone-600/10 text-[#5D4A3A] hover:text-stone-400 transition-colors disabled:opacity-50"
      ...
      {scheduledPosts.length === 0 && !generating && (
        <div className="text-center py-16">
          ...
          <h3 className="text-lg font-heading font-semibold text-[#5D4A3A]">No autopilot content yet</h3>
          <p className="text-sm text-[#5D4A3A] mt-1">Configure your preferences and click Generate to create a content calendar</p>
        </div>
      )}
    </div>
  );
