  ...
  return (
    <div className="space-y-6 animate-fade-in" data-testid="autopilot-page">
      <div>
        ...
        <p className="text-gray-500 mt-1">AI generates and schedules a full week of content across your platforms</p>
      </div>

      <div className="bg-white backdrop-blur-md border border-gray-200 rounded-xl p-6" data-testid="autopilot-config">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            ...
            <select value={tone} onChange={(e) => setTone(e.target.value)}
              className="w-full bg-gray-50 border border-gray-300 rounded-lg py-2.5 px-4 text-sm text-white transition-all" data-testid="autopilot-tone"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div>
            ...
            <select value={days} onChange={(e) => setDays(Number(e.target.value))}
              className="w-full bg-gray-50 border border-gray-300 rounded-lg py-2.5 px-4 text-sm text-white" data-testid="autopilot-days"
          ...
          <div>
            ...
            <select value={postsPerDay} onChange={(e) => setPostsPerDay(Number(e.target.value))}
              className="w-full bg-gray-50 border border-gray-300 rounded-lg py-2.5 px-4 text-sm text-white" data-testid="autopilot-ppd"
              ...
              {Object.entries(platformMeta).map(([key, meta]) => {
                ...
                return (
                  <button key={key} onClick={() => togglePlatform(key)}
                    className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium border transition-all ${active ? 'bg-gray-100 border-gray-300 text-white' : 'border-gray-200 text-gray-500'}`}
        ...
        <div className="space-y-4" data-testid="autopilot-calendar">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-heading font-semibold text-gray-900 flex items-center gap-2">
              <Calendar className="w-5 h-5 text-violet-600" /> Content Calendar ({scheduledPosts.length} posts)
            </h2>
          </div>
          {Object.entries(groupedByDay).sort().map(([day, posts]) => (
            <div key={day} className="space-y-2">
              <h3 className="text-sm font-heading font-semibold text-gray-700 flex items-center gap-2">
              ...
              {posts.map(post => {
                ...
                return (
                  <div key={post.post_id} className="bg-white backdrop-blur-md border border-gray-200 rounded-xl p-4 hover:border-gray-300 transition-colors" data-testid={`autopilot-post-${post.post_id}`}>
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`text-[10px] px-2 py-0.5 rounded-full ${post.status === 'published' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-violet-600/10 text-violet-600'}`}>
                            {post.status}
                          </span>
                          <span className="text-[10px] text-gray-400">{post.scheduled_at}</span>
                        </div>
                        <p className="text-sm text-gray-700 leading-relaxed line-clamp-3">{post.content}</p>
                        {post.hashtags?.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-2">
                            {post.hashtags.slice(0, 5).map((tag, i) => (
                              <span key={i} className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded">#{tag.replace('#','')}</span>
                            ))}
                          </div>
                        )}
                        {result && (
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        {post.status !== 'published' && (
                          <button onClick={() => handlePublishPost(post)} disabled={!!publishing}
                            className="p-2 rounded-lg hover:bg-emerald-500/10 text-gray-500 hover:text-emerald-400 transition-colors disabled:opacity-50"
      ...
      {scheduledPosts.length === 0 && !generating && (
        <div className="text-center py-16">
          ...
          <h3 className="text-lg font-heading font-semibold text-gray-500">No autopilot content yet</h3>
          <p className="text-sm text-gray-400 mt-1">Configure your preferences and click Generate to create a content calendar</p>
        </div>
      )}
    </div>
  );
