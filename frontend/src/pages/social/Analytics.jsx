  ...
  return (
    <div className="space-y-6 animate-fade-in" data-testid="analytics-page">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-heading font-bold text-[#4A3728] tracking-tight">Analytics & Listening</h1>
          <p className="text-[#5D4A3A] mt-1">Measure performance, track sentiment, and monitor your brand</p>
        </div>
        {tab === 'analytics' && (
          <button onClick={handleExportCSV} className="bg-[#E8D5C4] hover:bg-gray-200 text-white border border-[#D4BBA6] rounded-lg font-medium px-4 py-2 text-sm flex items-center gap-2 transition-all" data-testid="export-csv">
      ...
      <div className="flex gap-1 p-1 bg-white rounded-xl border border-[#E8D5C4]">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${tab === t.key ? 'bg-amber-800/15 text-amber-600' : 'text-[#5D4A3A] hover:text-[#4A3728] hover:bg-[#F5EDE5]'}`}
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {[
              ...
              <div key={m.label} className="bg-white border border-[#E8D5C4] rounded-xl p-4">
                <div className="flex items-center gap-1.5 mb-2"><m.icon className="w-3.5 h-3.5" style={{ color: m.color }} /><span className="text-[10px] text-[#5D4A3A] uppercase tracking-wider">{m.label}</span></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-white border border-[#E8D5C4] rounded-xl p-5">
              <h3 className="text-sm font-heading font-semibold text-[#4A3728] mb-4">Posts by Platform</h3>
            ...
            <div className="bg-white border border-[#E8D5C4] rounded-xl p-5">
              <h3 className="text-sm font-heading font-semibold text-[#4A3728] mb-4 flex items-center gap-2"><Zap className="w-4 h-4 text-amber-400" /> Industry Benchmarks</h3>
                ...
                {Object.entries(analytics?.benchmarks || {}).map(([platform, bench]) => {
                  ...
                  return (
                    <div key={platform} className="flex items-center gap-3 p-2.5 bg-[#F5EDE5] rounded-lg">
                      ...
                      {myStats && (
                        <div className={`flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full ${aboveBench ? 'bg-stone-600/10 text-stone-400' : 'bg-amber-500/10 text-amber-400'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[{ title: 'Top Performing', posts: analytics?.best_posts, icon: TrendingUp, color: 'text-stone-400' },
              { title: 'Needs Improvement', posts: analytics?.worst_posts, icon: TrendingDown, color: 'text-amber-400' }].map(section => (
              <div key={section.title} className="bg-white border border-[#E8D5C4] rounded-xl p-5">
        ...
        <div className="space-y-5">
          ...
          {listenResult && (
            <div className="space-y-4 animate-slide-up">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white border border-[#E8D5C4] rounded-xl p-5 flex flex-col items-center">
                  ...
                  <div className="relative w-24 h-24 mb-3">
                    ...
                    <span className="absolute inset-0 flex items-center justify-center text-xl font-heading font-bold text-[#4A3728]">{listenResult.sentiment_score || 50}</span>
                  </div>
                  <span className={`text-sm font-medium capitalize ${listenResult.overall_sentiment === 'positive' ? 'text-stone-400' : listenResult.overall_sentiment === 'negative' ? 'text-red-400' : 'text-amber-400'}`}>{listenResult.overall_sentiment}</span>
                </div>
                ...
                <div className="bg-white border border-[#E8D5C4] rounded-xl p-5">
                  ...
                  {listenResult.audience_mood && <p className="text-xs text-[#5D4A3A] mt-3 bg-[#F5EDE5] p-2 rounded-lg">{listenResult.audience_mood}</p>}
                </div>

                <div className="bg-white border border-[#E8D5C4] rounded-xl p-5">
                  <p className="text-[10px] text-[#5D4A3A] uppercase tracking-wider mb-3">Brand Health</p>
                  <div className="text-center">
                    <p className="text-4xl font-heading font-bold text-[#4A3728] mb-1">{listenResult.brand_health_score || 0}</p>
                  ...
                  {listenResult.hashtag_analysis?.length > 0 && (
                    <div className="mt-4">
                      ...
                      <div className="flex flex-wrap gap-1">{listenResult.hashtag_analysis.slice(0, 5).map((h, i) => (
                        <span key={i} className="text-[10px] bg-amber-800/10 text-amber-600 px-2 py-0.5 rounded">#{h.tag || h}</span>
                      ))}</div>
                    </div>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {listenResult.key_mentions?.length > 0 && (
                  <div className="bg-white border border-[#E8D5C4] rounded-xl p-5">
                    <h3 className="text-sm font-heading font-semibold text-[#4A3728] mb-3 flex items-center gap-2"><MessageSquare className="w-4 h-4 text-pink-400" /> Key Mentions</h3>
                    <div className="space-y-2">{listenResult.key_mentions.map((m, i) => (
                      <div key={i} className="p-2.5 bg-[#F5EDE5] rounded-lg">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-[10px] text-[#5D4A3A] capitalize">{m.platform}</span>
                          <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${m.sentiment === 'positive' ? 'bg-stone-600/10 text-stone-400' : m.sentiment === 'negative' ? 'bg-red-500/10 text-red-400' : 'bg-amber-500/10 text-amber-400'}`}>{m.sentiment}</span>
                        </div>
                        <p className="text-xs text-[#4A3728]">"{m.text}"</p>
                      </div>
                    ))}</div>
                  </div>
                )}
              ...
              {listenResult.recommendations?.length > 0 && (
                <div className="bg-white border border-[#E8D5C4] rounded-xl p-5">
                  <h3 className="text-sm font-heading font-semibold text-[#4A3728] mb-3 flex items-center gap-2"><Sparkles className="w-4 h-4 text-amber-400" /> AI Recommendations</h3>
          ...
          {!listenResult && !listening && (
            <div className="text-center py-16">
              ...
              <h3 className="text-lg font-heading font-semibold text-[#5D4A3A]">Monitor Your Brand</h3>
              <p className="text-sm text-[#5D4A3A] mt-1">Enter your brand name, product, or any topic to analyze sentiment and trending conversations</p>
            </div>
          )}
        </div>
      )}
