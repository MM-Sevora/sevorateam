    ...
    <div className="space-y-6 animate-fade-in" data-testid="youtube-explorer-page">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Search channels, analyze videos, and discover trending content</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setTab('search')}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'search' ? 'bg-red-500/10 text-red-400' : 'text-zinc-400 hover:text-white hover:bg-white/5'}`}
            data-testid="yt-tab-search"
          ><Search className="w-4 h-4 inline mr-1" /> Search</button>
          <button onClick={handleLoadTrending}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'trending' ? 'bg-red-500/10 text-red-400' : 'text-zinc-400 hover:text-white hover:bg-white/5'}`}
      ...
      {tab === 'search' && (
        <div className="space-y-4">
          <div className="bg-zinc-900/50 backdrop-blur-md border border-white/5 rounded-xl p-6">
            <div className="flex gap-3">
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                onKeyDown={handleKeyDown}
                className="flex-1 bg-zinc-950/50 border border-white/10 focus:border-red-500/50 focus:ring-2 focus:ring-red-500/20 rounded-lg py-2.5 px-4 text-sm text-white placeholder-zinc-500 transition-all"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" data-testid="yt-search-results">
              {searchResults.map((ch) => (
                <button key={ch.channel_id} onClick={() => handleSelectChannel(ch.channel_id)}
                  className="bg-zinc-900/50 backdrop-blur-md border border-white/5 rounded-xl p-4 text-left hover:border-white/10 hover:-translate-y-1 transition-all duration-200"
          ...
          {!loading && searchResults.length === 0 && !searchQuery && (
            <div className="text-center py-16">
              <FaYoutube className="w-12 h-12 text-zinc-700 mx-auto mb-4" />
              <h3 className="text-lg font-heading font-semibold text-zinc-400">Search YouTube Channels</h3>
              <p className="text-sm text-zinc-600 mt-1">Enter a channel name to explore analytics and videos</p>
            </div>
          )}
        </div>
      )}
      ...
      {tab === 'channel' && selectedChannel && (
        <div className="space-y-4">
          <div className="bg-zinc-900/50 backdrop-blur-md border border-white/5 rounded-xl overflow-hidden" data-testid="yt-channel-detail">
            <div className="p-6">
              <div className="flex items-start gap-4 mb-6">
                ...
                <div className="flex-1">
                  <h2 className="text-xl font-heading font-bold text-white">{selectedChannel.title}</h2>
                  {selectedChannel.custom_url && <p className="text-sm text-red-400">{selectedChannel.custom_url}</p>}
                  <p className="text-sm text-zinc-400 mt-1 line-clamp-2">{selectedChannel.description}</p>
                </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-zinc-950/50 rounded-lg p-4 border border-white/5 text-center">
                  <Users className="w-5 h-5 text-red-400 mx-auto mb-2" />
                  <p className="text-2xl font-heading font-bold text-white">{formatCount(selectedChannel.statistics?.subscribers)}</p>
                  <p className="text-xs text-zinc-500 mt-1">Subscribers</p>
                </div>
                <div className="bg-zinc-950/50 rounded-lg p-4 border border-white/5 text-center">
                  <Eye className="w-5 h-5 text-accent-cyan mx-auto mb-2" />
                  <p className="text-2xl font-heading font-bold text-white">{formatCount(selectedChannel.statistics?.total_views)}</p>
                  <p className="text-xs text-zinc-500 mt-1">Total Views</p>
                </div>
                <div className="bg-zinc-950/50 rounded-lg p-4 border border-white/5 text-center">
                  <Video className="w-5 h-5 text-accent-pink mx-auto mb-2" />
                  <p className="text-2xl font-heading font-bold text-white">{formatCount(selectedChannel.statistics?.video_count)}</p>
                  <p className="text-xs text-zinc-500 mt-1">Videos</p>
                </div>
              </div>
            </div>
          </div>

          <div className="bg-zinc-900/50 backdrop-blur-md border border-white/5 rounded-xl p-6" data-testid="yt-channel-videos">
            <h3 className="text-lg font-heading font-semibold text-white mb-4">Recent Videos</h3>
            {channelVideos.length > 0 ? (
              <div className="space-y-3">
                {channelVideos.map((vid) => (
                  <div key={vid.video_id} className="flex items-start gap-4 p-3 rounded-lg hover:bg-white/5 transition-colors" data-testid={`yt-video-${vid.video_id}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-zinc-500 mt-1">{new Date(vid.published_at).toLocaleDateString()}</p>
                    ...
                    <button onClick={() => handleVideoAnalytics(vid.video_id)}
                      className="flex-shrink-0 p-2 rounded-lg hover:bg-red-500/10 text-zinc-500 hover:text-red-400 transition-colors"
          ...
          {videoAnalytics && (
            <div className="bg-zinc-900/50 backdrop-blur-md border border-white/5 rounded-xl p-6 animate-slide-up" data-testid="yt-video-analytics">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-heading font-semibold text-white">Video Analytics</h3>
                <button onClick={() => setVideoAnalytics(null)} className="text-xs text-zinc-500 hover:text-zinc-300">Close</button>
              </div>
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500 mt-1">{videoAnalytics.channel_title} - {new Date(videoAnalytics.published_at).toLocaleDateString()}</p>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {videoAnalytics.tags.map((tag, i) => (
                      <span key={i} className="text-[10px] bg-zinc-800 text-zinc-400 px-2 py-1 rounded-md">{tag}</span>
                    ))}
                  </div>
                </div>
              )}
          ...
          {loading ? (
            ...
            <div className="space-y-3" data-testid="yt-trending-list">
              {trending.map((vid, index) => (
                <div key={vid.video_id} className="bg-zinc-900/50 backdrop-blur-md border border-white/5 rounded-xl p-4 flex items-start gap-4 hover:border-white/10 transition-colors" data-testid={`yt-trending-${index}`}>
                  <span className="text-lg font-heading font-bold text-zinc-600 w-8 flex-shrink-0 pt-1">#{index + 1}</span>
                  <div className="relative flex-shrink-0">
                    {vid.thumbnail && <img src={vid.thumbnail} alt={vid.title} className="w-40 h-24 rounded-lg object-cover" />}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-zinc-500 mt-1">{vid.channel}</p>
                    <div className="flex items-center gap-4 mt-2 text-xs text-zinc-400">
            ...
            <div className="text-center py-16">
              ...
              <h3 className="text-lg font-heading font-semibold text-zinc-400">Click Trending to load</h3>
            </div>
          )}
        </div>
      )}
